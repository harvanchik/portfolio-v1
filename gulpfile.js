...
function minNodeMods() {
	...
	return gulp.src(['./node_modules/theaterjs/dist/theater.min.js'])
		.pipe(minifyjs(options))
		.pipe(rev())
		.pipe(gulp.dest('./docs/assets/js'))
		.pipe(rev.manifest({ merge: true }))
...
function fixPaths() {
	return gulp.src(['./docs/**/*.html'])
		.pipe(replace('./node_modules/theaterjs/dist/theater.min.js', './assets/js/theater.min.js'))
		.pipe(gulp.dest('./docs'));
}

// Gulp default task (run by typing 'gulp' in the console)
gulp.task('default', gulp.series(clean, postCSS, minHTML, minJS, minNodeMods, copyImages, minSVG, fixPaths, revRewrite));
